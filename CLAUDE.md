...
> **綜合計算能力單元(2026-07 新增,巧算風格)**:在既有星球尾端各加一個「綜合」進階單元,課綱重點是**「簡便計算/巧算」——題目刻意設計,懂得用交換/結合/分配律/約分就能心算快速串出**(遊戲只判答案,不加教學提示——使用者決策)。
> - 燃燒星「整數綜合」第 29–31 關(數字鍵盤):29 加減·湊整(交換/結合)、30 乘除·湊整百千(招牌配對 25×4、125×8)、31 四則·分配律(a×m±a×c、a×99+a)。答案乾淨(整十/百/千倍),可到 100000(如 4×25×8×125)。
> - 海洋星「分數綜合」第 32–34 關(四選一,`kind:'frac'`):32 加減·湊成1(同分母好朋友)、33 乘除·連乘約分(倒數相消)、34 四則·分配律(提共同分數→整數答案)。**答案可為整數/假分數**(使用者以新範例覆蓋先前「保持分數」的決定);`buildFracOptions` 已擴充支援整數答案的四選一誘答。
> - 出題邏輯:整數用 `genIntCombo`、分數在 `genFrac` 加 case 32–34(皆模板+reject-sampling);保證答案為正、簡潔,且「算式真值(尊重先乘除後加減與括號) = 標準答案」(Node 3000 題驗證通過)。
